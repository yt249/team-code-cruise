...
import { CreateIntentBody, CreateRideBody, QuoteBody,ConfirmPaymentBody } from './validators.js';
...
router.post('/payments/intents', requireAuth, (req,res)=>{
  ...
  const intent = PaymentService.createPaymentIntent(ride.id, ride.fare_amount);
...
router.post('/payments/confirm', requireAuth, (req,res)=>{
  ...
  if(!parsed.success) return res.status(400).json(parsed.error.flatten());
  const out = PaymentService.confirmPayment(parsed.data.intentId, parsed.data.method);
  res.json({ status: out.status });
});

// Utility: update driver location (dev only)
router.post('/drivers/:id/location', (req,res)=>{
  const { lat, lon } = req.body || {};
  DriversRepo.updateLocation(req.params.id, Number(lat), Number(lon));
  res.json({ ok: true });
});